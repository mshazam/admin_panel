        ...
        <div class="main">
            ...
            <div class="table-responsive">
                <table class="table table-striped table-bordered table-hover">
                    ...
                    foreach($leave_types as $s):?>
                    <tr id="contentrow_<?=$s->getLeaveTypeId()?>">
                        <td>
                            <a class="pull-left" href="<?=site_url("dashboard/edit_leave_types/".$s->getLeaveTypeId())?>"><span class="btn btn-default  glyphicon glyphicon-pencil"></span></a>
                            <a class="pull-right deletebtn" href="#"  data-contentid="<?=$s->getLeaveTypeId()?>"><span class="btn  btn-default glyphicon glyphicon-trash"></span></a>
                        </td>
                        <td><?=$s->getLeaveTypeId()?></td>
                        <td><?=$s->getCountry()?></td>
                        <td><?=$s->getTimeAccountType()?></td>
                        <td><?=$s->getTranslation()?></td>
                        <td><?=$s->getEnabled()?></td>
                        <td><?=$s->getAttachment()?></td>
                    </tr>
                    <?endforeach;?>
                    </tbody>
                </table>
    ...
    $(function () {
        $('.deletebtn').click(function (e) {
            ...
            if (r == true) {
                $.ajax({
                    dataType: "json",
                    url : '<?=site_url("dashboard/deleteLeaveType")?>/'+content_id,
                    type: 'GET',
                    success : function(r){
