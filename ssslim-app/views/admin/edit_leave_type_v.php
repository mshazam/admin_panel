...
<?/** @var Ssslim\Libraries\Lead $leave_type?> */?>
<form id = "editLead" action="<?=site_url("dashboard/edit_leave_type/".$leave_type->getLeadId())?>" method="post" enctype="multipart/form-data">
    <div class="row">
    ...
    <fieldset name="contact">
        ...
        <div class="row">
            <div class="form-group col-md-4  <?=isset($errors['$this->fname'])?'has-error':''?> ">
                ...
                <input type="text" class="form-control" id="fname" name="fname" placeholder="First Name" value="<?= $leave_type->getFname()?>">
            </div>
            <div class="form-group col-md-4  <?=isset($errors['$this->lname'])?'has-error':''?> ">
                <label for="lname">Last Name</label>
                <input type="text" class="form-control" id="lname" name="lname" placeholder="Last Name" value="<?= $leave_type->getLname()?>">
            </div>
            <div class="form-group col-md-4  <?=isset($errors['email'])?'has-error':''?> ">
                <label for="email">Email</label>
                <input type="text" class="form-control" id="email" name="email" placeholder="Email" value="<?= $leave_type->getEmail()?>">
            </div>
        </div>
        <div class="row">
            <div class="form-group col-md-6  <?=isset($errors['$this->jobtitle'])?'has-error':''?> ">
                <label for="jobtitle">Job Title</label>
                <input type="text" class="form-control" id="jobtitle" name="jobtitle" placeholder="Job Title" value="<?= $leave_type->getJobtitle()?>">
            </div>
            <div class="form-group col-md-6  <?=isset($errors['company'])?'has-error':''?> ">
                <label for="company">Company</label>
                <input type="text" class="form-control" id="company" name="company" placeholder="Company" value="<?= $leave_type->getCompany()?>">
            </div>
        </div>
        ...
        <div class="row">
            <div class="form-group col-md-6  <?=isset($errors['State'])?'has-error':''?> ">
                ...
                <input type="text" class="form-control" id="state" name="state" placeholder="State" value="<?= $leave_type->getState()?>">
            </div>
            <div class="form-group col-md-6  <?=isset($errors['$this->country'])?'has-error':''?> ">
                <label for="country">Country</label>
                <input type="text" class="form-control" id="country" name="country" placeholder="Country" value="<?= $leave_type->getCountry()?>">
            </div>
        </div>
...
<script type="text/javascript">
    ...
    function doSendEmail() {
        ...
        $.ajax({
            // dataType: "json",
            url : '<?=site_url("admin/send_email_quote")?>/<?=$leave_type->getLeadId();?>',
            type: 'GET',
            success : function(r){
                if (r.s !== 0) {
    ...
    $(function () {
        ...
        $('#generatedTime').datetimepicker({format: 'YYYY-MM-DD HH:mm', defaultDate: '<?= $leave_type->getTransdate()?>'});
