...
class Route
{
    ...
    public function __construct($config = [])
    {
        $defaults = [
            ...
            'middleware'     => [],
            'classes'        => [
                'parser' => 'Ssslim\Core\Libraries\Router\Parser',
                'host'   => 'Ssslim\Core\Libraries\Router\Host'
            ]
        ];
        $config += $defaults;
    ...
    {
        if (Route::$_compiler_cache === null) {
            if (file_exists("routing.cache")) Route::$_compiler_cache = unserialize(file_get_contents("routing.cache"));
        }

        if (isset(Route::$_compiler_cache[$this->_pattern])) {
            $this->_regex = Route::$_compiler_cache[$this->_pattern][0];
            $this->_variables = Route::$_compiler_cache[$this->_pattern][1];
        }
        else {
            /** @var Parser $parser */
            $parser = $this->_classes['parser'];
            $rule = $parser::compile($this->token());
            $this->_regex = Route::$_compiler_cache[$this->_pattern][0] = $rule[0];
            $this->_variables = Route::$_compiler_cache[$this->_pattern][1] = $rule[1];
        }
    }
    ...
    static function commitCache() {
        file_put_contents("routing.cache", serialize(Route::$_compiler_cache));
    }
}
